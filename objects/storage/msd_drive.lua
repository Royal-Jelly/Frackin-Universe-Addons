...
function binInventoryChange()
  if entity.id() then
    ...
    local frames = configParameter("storage", 10)
    local fill = math.ceil(frames * fillPercent(container))
    if self.fill ~= fill then
      self.fill = fill
      setAnimationState("storage", tostring(fill))
    end
  end
end
...
function isn_makeBatteryDescription(desc, charge, onDeath)
	...
	str=str..((onDeath and " ^red;Interact for Info^reset;") or "\n^blue;Input 1^reset;: Power Switch\n^blue;Input^white;1^blue;: ^white;Item Network\n^red;Output^white;1^red;: ^white;item network")
